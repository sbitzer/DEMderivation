...
Following Friston we only consider Gaussian distributions and denote that the multidimensional random variable $\bs{x}$ is distributed according to a multivariate Gaussian distribution as $\bs{x} \sim \N(\bgs{\mu}, \Cov)$ where $\bgs{\mu}$ is the mean vector, $\Cov$ the covariance matrix and $\Prec = \Cov^{-1}$ the precision. The corresponding probability density function is
\eq{
    p(\bs{x}|\bgs{\mu},\Cov) = \frac{1}{\sqrt{\det{2\pi \Cov}}}\exp\left( -\frac{1}{2}(\bs{x} - \bgs{\mu})^\tr\Cov^{-1} (\bs{x} - \bgs{\mu}) \right)
}
which we may also write as
\eq{
    p(\bs{x}|\bgs{\mu},\Cov) = \frac{1}{Z(\Cov)}\exp\left( -\frac{1}{2}\ppe^\tr\Prec \ppe \right)
}
where we have defined the 'prediction error' $\ppe = \bs{x} - \bgs{\mu}$.
